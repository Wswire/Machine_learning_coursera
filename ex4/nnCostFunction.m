...
J = sum( sum((-yMat).*(log(a3))-((1-yMat).*log(1-a3)),2)); %+ (lambda/(2*m))*(theta(2:length(theta))'*theta(2:length(theta)));
J = J*(1/m);
...
d2 = d2(:,2:end);
d1 = (d2*Theta1).*sigmoidGradient(a1);  %5000*401


Theta1_grad = (1/m)*(d2'*a1);
Theta2_grad = (1/m)*(d3'*a2);
Theta1_grad(:, 2:end) = (Theta1_grad(:, 2:end) + (1/m)*lambda.*theta1);
